...
class Transaction {
  ...
  function generateRebroadcastTransaction(tid, slip_id, avg_fee=2) {
    ...
    if (this.transaction.type >= 0 && this.transaction.type <= 3) {
      ...
      let from = new saito.slip(this.transaction.to[slip_id].add, this.transaction.to[slip_id].amt, 3);
          from.tid = tid;
          from.sid = slip_id;
      let to   = new saito.slip(this.transaction.to[slip_id].add, amt.toFixed(8), 3);
  ...
  isRebroadcast(oldblk, newblk, slip_id) {
    ...
    //
    if (this.transaction.to[slip_id].add  == this.atr_trapdoor) {
