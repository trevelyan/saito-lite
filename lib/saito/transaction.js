...
class Transaction {
  ...
  validate(app, mode=0, bid=null) {
    ...
    ////////////////////
    for (let i = 0; i < this.transaction.from.length; i++) {
      if (!app.shashmap.validate_slip(this.transaction.from[i].toSignatureSource(), bid)) {
    ...
    ///////////////////////////
    let acceptable_lower_block_limit = block_id - app.blockchain.genesis_period;
    for (let i = 0; i < this.transaction.from.length; i++) {

      //
      // check input is within block limit
      //
      if (this.transaction.from[i].bid < acceptable_lower_block_limit && this.transaction.type == 0) {
      ...
      //
      // and check block hash on longest chain
      //
      if (this.transaction.from[i].bhash != "") {
