...
console.log("this should be in pos: " + pos);
...
console.log("FINDING LC: " + this.last_bid + " -- " + blk.block.prevbsh + " -- " + this.last_bsh);

    //
    // first two consecutive blocks is LC
    //
    if (this.last_bsh == undefined) {

      for (let i = 0; i < this.index.blocks.length; i++) {
        if (blk.block.prevbsh == this.index.blocks[i].returnHash()) {

	  let prevblk = this.index.blocks[i];
	  this.onChainReorganization(prevblk.returnHash(), prevblk.block.id, 1);

	  this.last_bsh = prevblk.returnHash();
	  this.last_bid = prevblk.block.bid;
	  this.last_bf = prevblk.block.bf;
	  this.last_ts = prevblk.block.ts;	  
...
console.log("LC POS IS HERE: " + pos);
    ...
    if (i_am_the_longest_chain == 1 && this.index.blocks.length == 1) {
      this.addBlockToBlockchainSuccess(blk, pos, 0);
      return;
    }
...
console.log("at the bottom of the blockchain addblocktoblockchain class");
