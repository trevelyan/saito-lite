...
Blockchain.prototype.saveBlockchain = async function saveBlockchain() {
  ...
  this.app.options.blockchain.fork_id      = this.returnForkId();
...
Blockchain.prototype.returnForkId = function returnForkId() {
  ...
  if (this.fork_id == "") {
    if (this.app.options.blockchain != undefined) {
      if (this.app.options.blockchain.fork_id != undefined) {
        if (this.app.options.blockchain.fork_id != "") {
          return this.app.options.blockchain.fork_id;
        }
      }
    }
  }
...
Blockchain.prototype.returnLatestBlockId = function returnLatestBlockId() {
  if (this.blocks.length == 0) {
    if (this.app.options.blockchain != undefined) {
      if (this.app.options.blockchain.last_bid != undefined) {
        return this.app.options.blockchain.last_bid;
      }
    }
    return 0; 
  }
...
console.log("About to Send Request for Missing Block: ");
  ...
  // should be safe to do as long as old_lc is changed before this point
  if (pos <= this.lc && this.index.lc.length > 1) {
    this.lc++;
    this.old_lc++;
  }
  ...
  //
    ...
    if (bid >= this.index.bid[this.lc]) {
      if (prevhash == this.index.hash[this.lc]) {
        ...
        while (search_pos >= 0) {
          ...
          //
            ...
            if (shared_ancestor_pos == 0) {
              if (nchain_prevhash != lchain_prevhash) {
                await this.addBlockToBlockchainSuccess(newblock, pos, 0);
                return;
              }
            }
        ...
        //
        if (nchain_len > lchain_len && nchain_bf >= lchain_bf && shared_ancestor_pos != -1) {
...
console.log(" .... success:     " + new Date().getTime());
  ...
  // save block to disk
  //
  // NOTE: we have moved this above Wallet.processPayment in order to
  // avoid having a second loop through the slips (and updating their
...
console.log(" .... run rmv txs: " + new Date().getTime());
  ...
  if (i_am_the_longest_chain == 1) {
    ...
    if (newblock.block.id%this.fork_id_mod == 0) { 
  ...
  if (i_am_the_longest_chain == 1) {
    //
    // propagateBlock when we know that it's saved to file and processed
    //
    this.app.network.propagateBlock(newblock);
  }
...
Blockchain.prototype.validateLongestChain = async function validateLongestChain(
  ...
  if (this.app.BROWSER == 1 || this.app.SPVMODE == 1) {
    ...
    for (let x = 0; x < old_block_ids.length; x++) {
      await this.app.storage.onChainReorganization(old_block_ids[x], old_block_hashes[x], 0);
    ...
    for (let x = 0; x < new_block_ids.length-1; x++) {
      await this.app.storage.onChainReorganization(new_block_ids[x], new_block_hashes[x], 1);
...
console.log(" .... block does not validate!");
  ...
  //
    ...
    if (blk == null) {
       ...
       //process.exit();
       //
       // this is untested, but should work - we do not unwind and rewind the chain
       // as we cannot be sure where it happened in the sequence. The most likely cause
       // is crafting of an invalid block.
       //
       await this.addBlockToBlockchainFailure(newblock, pos, i_am_the_longest_chain, force);
       return;
    }
...
Blockchain.prototype.returnMinTxId = function returnMinTxId() {
  ...
  if (this.index.mintid[this.lc] == undefined) { return 0; }
 ...
 * @param {array} keylist
 */
Blockchain.prototype.returnBlockWithTransactionsByHash = async function returnBlockWithTransactionsByHash(hash, keylist) {
  return new Promise((resolve, reject) => {
    this.returnBlockByHashFullOnlyIfContainsTransactionsWithCallback(hash, keylist, (blk) => {
      resolve(blk)
    });
  });
}
...
Blockchain.prototype.updateGenesisBlock = function updateGenesisBlock(blk, pos) {
  ...
  //
  // believe this is a bug -- should only be 1 genesis period plus the fork guard (+1 to ensure
  // there is a block to discard.
  //
  //if (blk.returnId() >= (this.genesis_bid + this.genesis_period + this.fork_guard)) {
  if (blk.returnId() >= (this.genesis_bid + this.fork_guard + 1)) {
...
Blockchain.prototype.hasFullGenesisPeriod = function hasFullGenesisPeriod() {

  let tmp_genesis_start = this.returnLatestBlockId() - this.genesis_period;

  if (this.index.bid.length > this.genesis_period) {
    if (tmp_genesis_start > this.lowest_acceptable_bid || (tmp_genesis_start <= 0 && this.lowest_acceptable_bid == 1)) {
      return true;
    }
  }
