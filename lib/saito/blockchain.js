...
console.log("ADDED: ");
console.log(JSON.stringify(this.index.blocks[pos].block));
...
console.log("HERE: " + i + " --- " + this.index.blocks.length);
console.log(JSON.stringify(this.index.blocks[i].block));
...
Blockchain.prototype.validateBlock = function validateBlock(blk) {
  console.log("VALIDATING: " + blk.returnHash());
  return 1;
}
Blockchain.prototype.validate = function validate(blk, pos, i_am_the_longest_chain, new_block_hashes, old_block_hashes) {

console.log("IN VALIDATE: " + JSON.stringify(new_block_hashes) + " -- " + JSON.stringify(old_block_hashes));
...
console.log("unwind chain -- " + blk.returnHash() + " --- " + JSON.stringify(old_block_hashes));
...
Blockchain.prototype.addBlockToBlockchainSuccess = async function addBlockToBlockchainSuccess(blk, pos, i_am_the_longest_chain, force=false) {
...
console.log(" .... post-save:   " + new Date().getTime());
  ...
  // - wallet - needs onchain reorganization
  // - callbacks
  // - modules
  //
...
Blockchain.prototype.saveBlockchain = function saveBlockchain(blk) {
