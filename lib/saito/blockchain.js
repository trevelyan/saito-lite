...
console.log("is our last HASH on the longest chain: " + this.bsh_lc_hmap[this.last_bsh]);
      //
...
console.log("I am longest chain: " + i_am_the_longest_chain);
...
console.log("SEARCH POS: " + search_pos);
            ...
            if (search_bsh == lchain_prevbsh && search_bsh == nchain_prevbsh) {
              ...
              search_pos = -1;
console.log("SHARED ANCESTOR: " + shared_ancestor_pos);
...
console.log("BBB");
    ...
    if (i_am_the_longest_chain == 1 && this.index.blocks.length == 1) {
      this.addBlockToBlockchainSuccess(blk, pos, 1);
      return;
    }
    ...
    if (i_am_the_longest_chain == 1 && this.index.blocks.length > 0) {
      ...
      new_block_hashes     = [];
      old_block_hashes     = [];
      if (this.last_lc_pos != pos) { old_hash_to_hunt_for = this.index.blocks[this.last_lc_pos].block.bsh; }
...
console.log("at the bottom of the blockchain addblocktoblockchain class: " + i_am_the_longest_chain);
  ...
  async validateBlock(blk) {

console.log("validating block: " + blk.returnHash());
    let block_validates = await blk.validate();
    if (!block_validates) {
      console.log("ERROR 0019423: block does not validate...");
      return 0;
    }
  ...
  validate(blk, pos, i_am_the_longest_chain, new_block_hashes, old_block_hashes) {
  ...
  async windChain(blk, pos, i_am_the_longest_chain, new_block_hashes, old_block_hashes, force, resetting_flag, current_wind_index) {
    ...
    if (blk.returnHash() === this_block_hash) {
      ...
      let block_validates = await this.validateBlock(blk);
...
console.log("updating LC_POS to : " + pos);
    ...
    // propagate block
    //
    // this.app.network.propagateBlock();
    //
    // permit addition of next block
    // wallet needs onchainreorg for first block to make things spendable
    //this.app.wallet.onChainReorganization(blk.block.id, blk.returnHash(), i_am_the_longest_chain);
    //
    this.indexing_active = 0;
  ...
  addBlockToBlockchainFailure(blk, pos, i_am_the_longest_chain, force=no) {
...
console.log("LCPOS: " + this.lc_pos);
  ...
  async returnBlockByHash(hash, mode=0) {
    ...
    //
console.log("CHECKING INDEX OF: " + hash);
    if (this.isHashIndexed(hash)) {
console.log("HASH INDEXED: 1");
      for (let v = this.index.blocks.length-1; v >= 0; v-- ) {
console.log("HASH INDEXED: 2");
        if (this.index.blocks[v].returnHash() == hash) {
          switch (mode) {
            case 0:
console.log("HASH INDEXED: 3 -- " + v);
console.log("returning: " + JSON.stringify(this.index.blocks[v].block));
              return this.index.blocks[v];
            case 1:
console.log("HASH INDEXED: 4");
	      if (this.index.blocks[v].transactions.length == 0) {
console.log("HASH INDEXED: 5");
                return await this.app.storage.loadBlockByHash(hash);
	      }
console.log("HASH INDEXED: 6");
              return this.index.blocks[v];
            default:
              return this.index.blocks[v];
          }
...
console.log("HASH INDEXED: 7");
  ...
  async returnBlockById(bid, mode=0) {
    ...
    // 1 -- require transactions
    //
    let hash = this.bid_bsv_hmap[bid];
