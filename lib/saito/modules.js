class Mods {
  ...
  ////////////////////////
  pre_initialize() {
    this.mods = this.mods_list.map(mod_path => {
  ...
  affixCallbacks(txindex, message, callbackArray, callbackIndexArray) {
    for (let i = 0; i < this.mods.length; i++) {
      if (message.module != undefined) {
        if (this.mods[i].shouldAffixCallbackToModule(message.module) == 1) {
console.log("AFFIX CALLBACK TO MODULE");
          callbackArray.push(this.mods[i].onConfirmation);
          callbackIndexArray.push(txindex);
        }
      } else {
        if (this.mods[i].shouldAffixCallbackToModule("") == 1) {
console.log("AFFIX CALLBACK TO MODULE 2");
          callbackArray.push(this.mods[i].onConfirmation);
          callbackIndexArray.push(txindex);
	}
      }
console.log("CBARRAYLENGTH: " + callbackArray.length);
    }
  }
