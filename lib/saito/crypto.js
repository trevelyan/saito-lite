...
//
Crypto.prototype.encodeXOR = function encodeXOR(plaintext, key) {
  return xor(new Buffer(plaintext, 'hex'), new Buffer(key, 'hex')).toString('hex');
}
Crypto.prototype.decodeXOR = function decodeXOR(str, key) {
  return xor(new Buffer(str, 'hex'), new Buffer(key, 'hex')).toString('hex');
};
Crypto.prototype.stringToHex = function stringToHex(str) {
  return new Buffer(str, 'utf-8').toString('hex');
}
Crypto.prototype.hexToString = function hexToString(hex) {
  return new Buffer(hex, 'hex').toString('utf-8');
}
Crypto.prototype.stringToBase64 = function stringToBase64(str) {
  return new Buffer(str, 'utf-8').toString('base64');
}
Crypto.prototype.base64ToString = function base64ToString(str) {
 ...
 */
Crypto.prototype.verifySignatureSource = function verifySignatureSource(msg, sig, pubkey) {
  try {
 ...
 * @returns {boolean} is signature valid?
 */
Crypto.prototype.fastVerifyMessage = function fastVerifyMessage(msg, sig, pubkey) {
  try {
    return secp256k1.verify(Buffer.from(msg, 'hex'), Buffer.from(this.fromBase58(sig),'hex'), Buffer.from(this.uncompressPublicKey(pubkey),'hex'));
  } catch (err) {
    console.log(err);
...
//
Crypto.prototype.fastSerialize = function fastSerialize(jsobj) {
