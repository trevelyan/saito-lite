...
BurnFee.prototype.returnWorkNeeded = function returnWorkNeeded(prevts=0, current_time=(new Date().getTime()), bf=this.bf) {
...
BurnFee.prototype.returnBurnFee = function returnBurnFee(prevblk=null, blk=null) {
  ...
  bf = Math.floor((prevblk.block.bf +1 ) * Math.sqrt((this.heartbeat * 1000)/(Math.abs(blk.block.ts - prevblk.block.ts) + 1)));
