...
BurnFee.prototype.returnWorkNeeded = function returnWorkNeeded(prevts=0, current_time=(new Date().getTime()), bf=0) {
...
BurnFee.prototype.returnBurnFee = function returnBurnFee(prevblk=null, blk=null) {
  ...
  let bf = Math.floor((prevblk.block.bf +1 ) * Math.sqrt((this.heartbeat * 1000)/(Math.abs(blk.block.ts - prevblk.block.ts) + 1)));
  return bf;
