...
*/
  ...
  handleHandshakeRequest(message) {
    ...
    if (this.handshake.challenge_mine != "" && peershake.challenge_proof != "" && this.handshake.challenge_verified == 0) {
      if (this.app.crypto.verifyMessage("_" + this.handshake.challenge_mine, peershake.challenge_proof, this.peer.publickey) == 0) {
      } else {
    ...
    for (let i = start_idx; i < this.app.blockchain.index.blocks.length; i++) {
      ...
      // if (this.app.blockchain.index.blocks[i].block.lc == 1) {
        if (message.data.start == null) {
          message.data.start = this.app.blockchain.index.blocks[i].block.prevbsh;
        }

        message.data.prehash.push(this.app.blockchain.index.blocks[i].prehash);
        message.data.bid.push(this.app.blockchain.index.blocks[i].block.bid);
        message.data.ts.push(this.app.blockchain.index.blocks[i].block.ts);
