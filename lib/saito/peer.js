...
class Peer {
  ...
  sendHandshake(sync_blockchain = 1) {
    ...
    request.data.last_bid = this.app.blockchain.last_bid;
    request.data.forkid = this.app.blockchain.fork_id;
    request.data.genesis_bid = this.app.blockchain.genesis_bid;
...
alert("NOT YET IMLEMENTED ADD HASH TO BLOCKCHAINS");
process.exit();
