...
function Mempool(app) {
  ...
  // hashmap
  //
  this.blocks_hmap        	= [];  // index is blk.returnHash()
  this.transactions_hmap        = [];  // index is tx.transaction.sig
  this.transactions_inputs_hmap = [];  // index is slip returnIndex()


  //
  // allow mempool to respond to action
  //
  this.app.connection.on('add_block_to_mempool', function(error, response) {
console.log("ADD BLOCK TO MEMPOOL: " + response);
  });
...
Mempool.prototype.canBundleBlock = function canBundleBlock() {
  ...
  console.log("RWN: " + this.routing_work_needed);
...
//
// TODO - must fix this if golden ticket cannot count unless target correct
//
...
Mempool.prototype.addBlock = function addBlock(blkjson) {

  console.log("able to produce a block!");


  let blk = new saito.block(blkjson);
...
Mempool.prototype.bundleBlock = async function bundleBlock() {
  ...
  // ensure not bundling
  if (this.app.monitor.canBundleBlock() == false) { return; }
