...
console.log("FETCHED A BLOCK: " + JSON.stringify(prevblk.block));

  if (prevblk != null) {
console.log("THIS IS PREV BLOCK: "+prevblk.block.ts);
  ...
  // HACK FOR BURN FEE
  //
  if (this.routing_work_needed == 1) { this.routing_work_needed = 0; }
...
Mempool.prototype.removeTransaction = function removeTransaction(tx=null) {
  ...
  for (let i = 0; i < tx.transaction.from.length; i++) {
    delete this.transactions_inputs_hmap[tx.transaction.from[i].returnSignatureSource()];
  }
