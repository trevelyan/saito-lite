...
Block.prototype.returnHash = function returnHash() {
  ...
  this.prehash = this.app.crypto.hash(this.returnSignature());
...
Block.prototype.returnSignature = function returnSignature() {
  ...
  for (let b = 0; b < this.transactions.length; b++) {
    for (let bb = 0; bb < this.transactions[b].transaction.from.length; bb++) {
      if (this.transactions[b].transaction.from[bb].amt > 0) {
        this.app.shashmap.insert_slip(this.transactions[b].transaction.from[bb].returnSignature(), this.block.id);
      }
    }
  }
  return 1;
...
Block.prototype.unspendInputs = function unspendInputs() {
  for (let b = 0; b < this.transactions.length; b++) {
    for (let bb = 0; bb < this.transactions[b].transaction.from.length; bb++) {
      if (this.transactions[b].transaction.from[bb].amt > 0) {
        this.app.shashmap.insert_slip(this.transactions[b].transaction.from[bb].returnSignature(), -1);
      }
    }
  }
  return 1;
