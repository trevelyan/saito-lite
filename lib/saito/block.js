...
console.log("RESTORING TXS WITH: " + txs[i]);
  ...
  async bundle(prevblk=null) {
	...
	if (number_of_golden_tickets > 1) {
	  ...
	  console.log("GT: " + JSON.stringify(this.transactions[i].transaction.msg, null, 4));
	  console.log("PO: " + JSON.stringify(payments, null, 4));

	  let slips = [];
	  for (let i = 0; i < payments.blocks.length; i++) {

	    if (payments.blocks[i].producer_share !== "0.0" && payments.blocks[i].producer_share !== "0") {
	      slips.push(new saito.slip(payments.blocks[i].producer_publickey, payments.blocks[i].producer_share, 1));
      	    }
      	    if (payments.blocks[i].miner_share !== "0.0" && payments.blocks[i].miner_share !== "0") {
      	      slips.push(new saito.slip(payments.blocks[i].miner_publickey, payments.blocks[i].miner_share, 1));
      	    }
      	    if (payments.blocks[i].router_share !== "0.0" && payments.blocks[i].router_share !== "0") {
              slips.push(new saito.slip(payments.blocks[i].router_publickey, payments.blocks[i].router_share, 1));
      	    }
      	    if (payments.blocks[i].staker_share !== "0.0" && payments.blocks[i].staker_share !== "0") {
	    ...
	    //
	    // and add the sequential transaction id
	    //
	    this.transactions[this.transactions.length-1].transaction.id = this.transactions[this.transactions.length-2].transaction.id+1;
...
console.log(JSON.stringify(this.transactions[i].transaction));
...
console.log(" ... calculating payouts for block " + this.block.id + " -- " + rloop);
	...
	if (rloop == 10) {
          rvalue.treasury = Big(rvalue.treasury).plus(Big(payments.total_fees_in_block)).toFixed(8);
	  return rvalue;
	} else {
	  return rvalue;
	}
    ...
    if (this.block.prevbsh == "") {
      if (rloop == 10) {
        rvalue.treasury = Big(rvalue.treasury).plus(Big(this.returnFees())).toFixed(8);
        return rvalue;
      } else {
        return rvalue;
      }
