...
console.log("ADDED: " + JSON.stringify(newtx.transaction));
  ...
  async validate() {
	...
	return 0;
      ...
      let hmap = [];
      let number_of_golden_tickets = 0;
      let number_of_block_payments = 0;
      for (let i = 0; i < this.transactions.length; i++) {
	  ...
	  if (this.transactions[i].transaction.msg.target != prevblk.returnHash()) {
	    ...
	    number_of_golden_tickets++;
	    if (number_of_block_payments > 1) {
	      console.log("ERROR 189842: two golden tickets in block");
	      return 0;
	    }

	    //
	    // check payments are consistent
	    //
	    for (let z = 0; z < this.transactions.length; z++) {
	      if (this.transactions[z].transaction.type == 2) {

		number_of_block_payments++;
		...
		// check from payments
		//
		if (this.app.crypto.hash(JSON.stringify(slips)) == this.app.crypto.hash(JSON.stringify(this.transactions[z].transaction.to))) {
		  console.log("\n\n\nTO SLIPS IN FEE TRANSACTION VALID!\n\n");
		} else {
