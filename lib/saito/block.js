...
class Block {
  ...
  async validate() {
	...
	return 0;
      ...
      let bf = this.app.burnfee.returnBurnFee(prevblk, this.block.ts);
      if (this.block.bf != bf) {
	console.log("ERROR 571928: block difficulty does not validate in new block");
	return 0;
      }
	  ...
	  return 0;
        ...
        // golden ticket
        //
        if (this.transaction[i].transaction.type == 1) {
	  if (this.transactions[i].transaction.msg.target != prevblk.returnHash()) {
        ...
        for (let j = 0; j < this.transactions[i].transaction.from.length; j++) {
          ...
          hmap[this.transactions[i].transaction.from[j].returnIndex()] = 1;
        }
      ...
      if (this.app.blockchain.isFullySynced() == 1) {
        ...
        let eblk	= await this.app.blockchain.returnBlockById((this.block.id - this.app.blockchain.genesis_period - 1);
