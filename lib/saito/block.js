...
console.log("length: " + this.callbacks.length);
    }
  }


  async runCallbacks(conf) {
console.log("rcb -- " + this.confirmations + " -- " + conf);
    for (let i = this.confirmations + 1; i <= conf; i++) {
console.log("rcb -- " + this.callbacks.length);
      for (let ii = 0; ii < this.callbacks.length; ii++) {
        this.callbacks[ii](this, this.transactions[this.callbacksTx[ii]], i, this.app);
      }
    }
console.log("setting rbc confirmations to " + conf + " for " + this.returnHash());
...
console.log("A10");
  ...
  async returnBlockPayouts(gt=null) {
