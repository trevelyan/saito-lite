...
class Miner {
  ...
  */
  startMining(blk) {
    ...
    //
    // don't mine if browser and only have 1 slip
    //
    if (this.app.wallet.wallet.inputs.length < 3 && this.app.BROWSER == 1) {
      return;
    }
...
async attemptSolution(blk) {
  ...
  if (!blk.is_valid) { return; }
  //
  // we do not mine on block #1, as it contains
  // VIP transactions and that can cause problems
  // as there is no golden ticket.
  //
  ...
  if (gt.isValidSolution(proposedSolution, blk)) {
    this.stopMining();
    let gtobj = gt.calculateSolution(blk, publickey, privatekey, randomNumber);
    let winning_node = gt.findWinner(gtobj, blk);
    ...
    //
    //
    //  Richard Got this Far
    //
    prevblk = this.app.blockchain.returnBlockByHash(blk.prevhash, 0)
    let total_fees_needed_for_blk    = await this.app.burnfee.returnWorkNeeded(prevblk, blk, blk.bf);
    let total_fees_available_for_creator = blk.returnAvailableFees(blk.block.creator);
    let total_fees_in_blk            = blk.returnTotalFees();
    let creator_surplus			 = Big(total_fees_available_for_creator).minus(Big(total_fees_needed_for_blk));
    let total_fees_for_miners_and_nodes  = Big(total_fees_in_blk).minus(creator_surplus).plus(blk.returnCoinbase());
    ...
    // from slips
    //
    // to avoid problems we require TWO input slips to start paying mining fees
    //
    if (Big(this.app.wallet.returnAvailableInputs()).gte(this.app.wallet.returnDefaultFee()) && this.app.wallet.wallet.inputs.length > 1) {
    ...
    // change address
    //let change_amount = total_submitted.minus(Big(this.app.wallet.returnDefaultFee()));
    //
    // set a low fee for testing
