...
class Miner {
  ...
  async mine(blk) {
    ...
    if (gt.isValidSolution(sol, blk)) {
      ...
      let payments = await blk.returnBlockPayouts(gtmsg);

      let slips = [];
      if (payments.blocks[0].producer_share !== "0.0" && payments.blocks[0].producer_share !== "0") {
        slips.push(new saito.slip(payments.blocks[0].producer_publickey, payments.blocks[0].producer_share, 1));
      }
      if (payments.blocks[0].miner_share !== "0.0" && payments.blocks[0].miner_share !== "0") {
        slips.push(new saito.slip(payments.blocks[0].miner_publickey, payments.blocks[0].miner_share, 1));
      }
      if (payments.blocks[0].router_share !== "0.0" && payments.blocks[0].router_share !== "0") {
        slips.push(new saito.slip(payments.blocks[0].router_publickey, payments.blocks[0].router_share, 1));
      }
      if (payments.blocks[0].staker_share !== "0.0" && payments.blocks[0].staker_share !== "0") {
        slips.push(new saito.slip(payments.blocks[0].staker_publickey, payments.blocks[0].staker_share, 1));
      }
      ...
      for (let i = 0; i < slips.length; i++) {
        slips[i].sid = i;
        tx.transaction.to.push(slips[i]);
      }
