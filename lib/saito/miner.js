'use strict';
const saito = require('./saito');
const Big = require('big.js');


function Miner(app) {

  if (!(this instanceof Miner)) {
    return new Miner(app);
  }

  this.app                = app || {};

  this.mining_active    = false;
  this.mining_speed     = 200;
  this.mining_timer     = null;

  return this;

}
module.exports = Miner;


Miner.prototype.startMining = function startMining(prevblk) {

  if (this.app.options.mining == 0) { return; }
  if (prevblk == null) { return; }
  if (!prevblk.is_valid) { return; }

console.log("starting to mine!");

  if (this.mining_active) { clearInterval(this.mining_timer); }
  this.mining_active = true;

  this.mining_timer = setInterval(() => {
    this.mine(prevblk);
  }, this.mining_speed);

}


Miner.prototype.stopMining = function stopMining() {
  this.mining_active = false;
  clearInterval(this.mining_timer);
}


Miner.prototype.mine = async function mine(blk) {

  //
  // browsers don't mine if it exhausts slip capacity.
  //
  if (this.app.BROWSER == 1) {
    if (this.app.wallet.wallet.inputs.length < 2) {
      return;
    }
  }

  if (blk == null) { return; }
  if (!blk.is_valid) { return; }
  // do not mine on block #1 because VIP txs
  if (blk.block.id == 1) { return; }

  //
  // generate random hash
  //
  // TODO longer random number as strong
  //
  let num = Math.random().toString();
  let sol = this.app.crypto.hash(this.app.wallet.returnPublicKey() + num);

  //
  // create golden ticket 
  //
  let gt = new saito.goldenticket(this.app);
  if (gt.isValidSolution(sol, blk)) {

console.log("rh: " + num + " -- " + sol);

    this.stopMining();

    //
    // so who do we pay ?
    //
    let gtmsg = gt.formatSolutionForGoldenTicket(blk, num);
    let payments = blk.returnBlockPayouts(gtmsg);

console.log("PAYOUT: " + JSON.stringify(payments));

    let slips = [];
    if (payments.producer_share != "0.0") {
      slips.push(new saito.slip(payments.producer_publickey, payments.producer_share, 1));
    }
    if (payments.miner_share != "0.0") {
      slips.push(new saito.slip(payments.miner_publickey, payments.miner_share, 1));
    }
    if (payments.router_share != "0.0") {
      slips.push(new saito.slip(payments.router_publickey, payments.router_share, 1));
    }
    if (payments.staker_share != "0.0") {
      slips.push(new saito.slip(payments.staker_publickey, payments.staker_share, 1));
    }

    //
    // create golden ticket payouts
    //
    let tx = new saito.transaction();
    tx.transaction.ts  = new Date().getTime();
    tx.transaction.type = 1;
    tx.transaction.msg = gtobj;

    //
    // from slips
    //
    if (this.app.wallet.wallet.inputs.length > 1) {
      tx.transaction.from = this.app.wallet.returnAdequateInputs(0.0001);
      if (tx.transaction.from == null) { return; }
    } else {
      tx.transaction.from = [];
      tx.transaction.from.push(new saito.slip(this.app.wallet.returnPublicKey(), 0.0, 1));
    }

    //
    // to slips
    //
    tx.transaction.to = [];
    for (let i = 0; i < slips.length; i++) {
      slips[i].sid = i;
      tx.push(slips[i]);
    }

    //
    // add change address
    //
    let total_submitted = Big(tx.transaction.from[0].amt);
    for (let r = 1; r < tx.transaction.from.length; r++) {
      total_submitted = total_submitted.plus(Big(tx.transaction.from[r].amt));
    }

    let change_amount = total_submitted.minus(Big(0.0001));
    if (change_amount.gt(0)) {
      tx.transaction.to.push(new saito.slip(this.app.wallet.returnPublicKey(), change_amount.toFixed(8), 0));
      tx.transaction.to[tx.transaction.to.length-1].type = 0;
    }

    tx = this.app.wallet.signTransaction(tx);
    this.app.wallet.saveWallet();

console.log("THIS IS OUR MINER GOLDEN TICKET: \n\n" + JSON.stringify(tx));

  }
}



