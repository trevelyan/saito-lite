...
class Network {
  ...
  propagateTransaction(tx, outbound_message="transaction", mycallback=null) {
	...
	//
	//
	console.log("ERROR 581323: asked to propagate a block we do not have in our mempool, add functionality to avoid block spamming");
	return;
...
//  }
  ...
  sendTransactionToPeers(tx, outbound_message, fees=1, callback=null) {
    this.peers.forEach((peer) => {
      if (!peer.inTransactionPath(tx) && peer.returnPublicKey() != null && fees >= peer.peer.minfee) {
  ...
  returnPeerByPublicKey(pkublickey) {
    for (let i = 0; i < this.peers.length; i++) { if (this.peers[i].peer.publickey == publickey) { return this.peers[i]; } }
    return null;
  }
